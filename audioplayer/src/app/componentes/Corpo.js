                ...
                <aside className='h-full w-50 bg-background p-1 rounded-lg'>
                    <nav className='h-full w-full'>
                        <ul className='flex flex-col gap-5 pt-10'>
                            {linkNav.map((item) => (
                                <li key={item.name} className='w-full h-10 pl-5 hover:bg-[#262626] transition-colors rounded'><Link href={item.path} className='flex gap-5 items-center w-full h-full'><Image src={currentPath === item.path ? item.iconAtivo : item.icon } className='w-4 h-4' alt="" /><span className='text-gray-400'>{item.name}</span></Link></li>
                            ))}
                        </ul>
                    </nav>
                </aside>
